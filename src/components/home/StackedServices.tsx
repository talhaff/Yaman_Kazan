      ...
      <div className="container mx-auto px-6 max-w-7xl">
        ...
        <div className="relative space-y-6 md:space-y-8">
          {services.map((service, index) => (
            <ServiceCard key={index} service={service} index={index} total={services.length} t={t} language={language} />
          ))}
        </div>
      </div>
    </section>
...
function ServiceCard({ service, index, total, t, language }: { service: typeof services[0], index: number, total: number, t: any, language: string }) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-24 items-center w-full relative z-10">
          ...
          <div className="lg:col-span-7 flex flex-col items-start md:pl-12">
            <div className="mb-4 md:mb-6 flex items-center gap-4">
              ...
              <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] opacity-80">{t("services.serviceArea")}</span>
            </div>
